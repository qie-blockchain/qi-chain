...
interface IDistribution {
    function refPayout(address _user) external payable;
}
...
contract OpenMarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder, ReentrancyGuard {
    ...
    function setDistributionAddress(address _distributionContract) public onlyOwner {
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) external payable whenNotPaused {
        ...
        if (FeeManager.cutPerMillion > 0) {
            ...
            if(order.currency == MARKETPLACE_ETHER) {
                
                uint256 adminShare = saleShareAmount.div(2);
                uint256 uplineShare = saleShareAmount.sub(adminShare);
                
                payable(owner()).transfer(adminShare);
                IDistribution(distributionContract).refPayout{value:uplineShare}(order.seller);
        ...
        if (order.royalty != 0) {
            ...
            if(INFT(order.nftAddress).getTokenMinter(_assetId) != msg.sender)
            {
               // Transfer royalty percentage for marketplace Owner
                order.currency == MARKETPLACE_ETHER ? 
                payable(address(INFT(order.nftAddress).getTokenMinter(_assetId))).transfer(royaltyAmount)
                :
                IERC20(order.currency).safeTransferFrom(msg.sender, address(INFT(order.nftAddress).getTokenMinter(_assetId)), royaltyAmount);
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if (FeeManager.cutPerMillion > 0) {
            ...
            // Transfer share amount for marketplace Owner
            if(order.currency == MARKETPLACE_ETHER) {
                // address(distributionContract).transfer(saleShareAmount);
                // IDistribution(distributionContract).refPayout{value:saleShareAmount}(order.seller);
        ...
        if (order.royalty != 0) {
            ...
            if(INFT(order.nftAddress).getTokenMinter(_assetId) != bid.bidder)
            {
                // Transfer royalty percentage for marketplace Owner
                order.currency == MARKETPLACE_ETHER ? 
                payable(address(INFT(order.nftAddress).getTokenMinter(_assetId))).transfer(royaltyAmount)
                :
                IERC20(order.currency).safeTransfer(address(INFT(order.nftAddress).getTokenMinter(_assetId)), royaltyAmount);
    ...
    function _executeOrder(bytes32 _orderId, address _buyer, address _nftAddress, uint256 _assetId, uint256 _priceInWei) internal {
        ...
        // Transfer NFT asset
        INFT(_nftAddress).safeTransferFrom(address(this), _buyer, _assetId);
    ...
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _currency) internal {
        // Check nft registry
        INFT nftRegistry = INFT(_nftAddress);
        ...
        orderByAssetId[_nftAddress][_assetId] = Order({
            ...
            price: _priceInWei,
            royalty: nftRegistry.getRoyaltyAmount(_assetId),
    ...
    function _cancelOrder(bytes32 _orderId, address _nftAddress, uint256 _assetId, address _seller) internal {
        ...
        INFT(_nftAddress).safeTransferFrom(address(this), _seller, _assetId);
