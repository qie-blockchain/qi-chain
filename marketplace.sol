...
interface INFT {
    function getMetaDataHash(string memory _metaData) external pure returns (bytes32);
...
contract FeeManager is Ownable {
    ...
    uint256 public constant maxCutPerMillion = 200000; // 10% cut
...
contract OpenMarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => address)) private nftToCreator;
    ...
    function addNFT(address _nftAddress, uint256 _nftId, address _creator) external onlyOwner {
        require(_nftAddress != address(0), "Invalid NFT Address");
        require(_creator != address(0), "Invalid Creator Address");

        // Check if the NFT ID for the given address is not already set
        require(nftToCreator[_nftAddress][_nftId] == address(0), "NFT already exists");

        // Add NFT information to the mapping
        nftToCreator[_nftAddress][_nftId] = _creator;

        // Emit event
        // emit NFTAdded(_nftAddress, _nftId, _creator);
    }
    /**
     ...
     * @param _currency - Currency for which the order is created
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei,uint256 _royalty, uint256 _expiresAt, address _currency) external whenNotPaused {
        _createOrder(_nftAddress, _assetId, _priceInWei, _royalty, _expiresAt, _currency);
    }
     function createOrderAdmin(address _nftAddress, uint256 _assetId, uint256 _priceInWei,uint256 _royalty, uint256 _expiresAt, address _currency, address _actualOwner) external whenNotPaused {
        _createOrderAdmin(_nftAddress, _assetId, _priceInWei, _royalty, _expiresAt, _currency, _actualOwner);
    }
    ...
    function getTokenMinterTesting(address _nftAddresss, uint256 _assetId) external view returns(address){
    address returnAddress = INFT(_nftAddresss).getTokenMinter(_assetId);
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) external payable whenNotPaused {
        ...
        // Send royalty to minter
        if (order.royalty != 0) {
            // Calculate royalty share
            royaltyAmount = order.price.mul(order.royalty).div(
                10000
            );

            if(INFT(order.nftAddress).getTokenMinter(_assetId) != msg.sender || nftToCreator[order.nftAddress][_assetId] != msg.sender)
            {
               // Transfer royalty percentage for marketplace Owner
                  address recipient = nftToCreator[order.nftAddress][_assetId];

                // If msg.sender is not the creator, set the recipient as the minter
             if (recipient == address(0)) {
                 recipient = address(INFT(order.nftAddress).getTokenMinter(_assetId));
                }

                 // Transfer royalty to the determined recipient
                 order.currency == MARKETPLACE_ETHER ?
                payable(recipient).transfer(royaltyAmount)
                :
                IERC20(order.currency).safeTransferFrom(msg.sender, recipient, royaltyAmount);
    ...
    function safePlaceBidAdmin(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _actualBidder)
        external payable whenNotPaused nonReentrant onlyOwner {

        Order memory order = _getValidOrder(_nftAddress, _assetId);

        order.currency == MARKETPLACE_ETHER ? 
            _createBidAdmin(_nftAddress, _assetId, msg.value, _expiresAt, _actualBidder)
            :
            _createBidAdmin(_nftAddress, _assetId, _priceInWei, _expiresAt, _actualBidder);
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if (order.royalty != 0) {
            ...
            royaltyAmount = bid.price.mul(order.royalty).div(
                10000
            );
        ...
        order.currency == MARKETPLACE_ETHER ? 
            payable(order.seller).transfer(bid.price.sub(saleShareAmount).sub(royaltyAmount))
            :
            IERC20(order.currency).safeTransfer(order.seller, bid.price.sub(saleShareAmount).sub(royaltyAmount));
     ...
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei,uint256 _royalty, uint256 _expiresAt, address _currency) internal {
        // Check nft registry
        ...
        // nftRegistry.approve(address(this), tokenId)
    ...
    function _createOrderAdmin(address _nftAddress, uint256 _assetId, uint256 _priceInWei,uint256 _royalty, uint256 _expiresAt, address _currency, address _toUser) internal {
        // Check nft registry
        IERC721 nftRegistry = IERC721(_nftAddress);
        ...
        // get NFT asset from seller
        nftRegistry.safeTransferFrom(msg.sender, address(this), _assetId);
    ...
    function _createBidAdmin(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _actualBidder) internal {
        // Checks order validity
        Order memory order = _getValidOrder(_nftAddress, _assetId);
        ...
        // Transfer sale amount from bidder to escrow
        // acceptedToken.safeTransferFrom(_actualBidder, address(this), _priceInWei);

        if(order.currency != 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE){
            IERC20(address(order.currency)).transferFrom(msg.sender, address(this), _priceInWei);
        }
        // Create bid
        bytes32 bidId = keccak256(abi.encodePacked(block.timestamp, _actualBidder, order.id, _priceInWei, _expiresAt));

        // Save Bid for this order
        bidByOrderId[_nftAddress][_assetId] = Bid({
            id: bidId,
            bidder: _actualBidder,
            price: _priceInWei,
            expiresAt: _expiresAt
        });

        emit BidCreated(bidId, _nftAddress, _assetId, _actualBidder, _priceInWei, _expiresAt);
    }
