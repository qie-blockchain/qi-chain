...
contract NFT1 is Ownable, ERC721 {
    ...
    function create(
        ...
        external
    {
        _create(_metaDataURI, _metaData, _royaltyAmount);
    }
    ...
    function createAdmin(
        ...
        string calldata _metaData,
        uint256 _royaltyAmount,
        address _actualCreator,
        uint256 _actualTokenId
    )
        external onlyOwner
    {
        _createAdmin(_metaDataURI, _metaData, _royaltyAmount,_actualCreator,_actualTokenId);
    }
    ...
    function _createAdmin(
        ...
        internal onlyOwner returns (uint256 tokenId)
    {
        tokenId = _actualTokenId;

        require(_royaltyAmount <= maxRoyalty);

        /// Save data
        extraInfoMap[tokenId] = TokenExtraInfo({
            metaDataURI: _metaDataURI,
            metaDataHash: getMetaDataHash(_metaData),
            tokenMinter: _actualCreator,
        ...
        emit ItemCreated(_actualCreator, tokenId);
    }
    function _create(
    ...
    function updateMaxRoyalty(uint256 _maxRoyalty) external onlyOwner {
