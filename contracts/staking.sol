...
contract QIDexStaking is Ownable, ReentrancyGuard, IQIDexStaking {
    ...
    function setAccTokenPerShare(uint256 newValue)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function deposit() external payable override nonReentrant {
        ...
        for (uint256 j = 0; j < stakes.length; j++) {
            ...
            stake.rewardDebt = stake.amount.mul(accTokenPerShare).div(
                PRECISION_FACTOR
            );
        }
    ...
    function depositAdmin(
        ...
        _addStakeAdmin(_actualStaker, _amount , _duration, _end, _rewardDebt);
        ...
        emit DepositAdmin(_actualStaker, _amount, _duration, _end, _rewardDebt);
    }
    ...
    function _addStake(address _account, uint256 _amount) internal {
        ...
        newStake.rewardDebt = newStake.amount.mul(accTokenPerShare).div(
            PRECISION_FACTOR
        );
    }

    function _addStakeAdmin(
        address _account,
        uint256 _amount,
        uint256 _duration,
        uint256 _end,
        uint256 _rewardDebt) internal {
        Stake[] storage stakes = userStakes[_account];
        ...
        newStake.end = _end;
        newStake.amount = _amount;
        newStake.rewardDebt = _rewardDebt;
    }
    ...
    function updateFee(uint256 _depositFee, uint256 _withdrawFee)
        external
        onlyOwner
    {
        depositFee = _depositFee;
